...
export const USERNAME_MESSAGE = '영문 소문자와 숫자만으로 4~20자를 입력해 주세요.'
export const PASSWORD_MESSAGE = '영문, 숫자, 특수문자를 혼합하여 8~20자로 입력해 주세요.'

/** 아이디 검증 — 통과 시 null, 실패 시 안내 메시지 반환. */
export function validateUsername(value: string): string | null {
  if (value.length === 0) return null // 미입력 상태에선 에러 미표시
  return USERNAME_REGEX.test(value) ? null : USERNAME_MESSAGE
}
...
/**
 * 아이디/비밀번호/비밀번호확인을 실시간 검증하는 커스텀 훅.
 * - 값 변경 시마다 정규식으로 즉시 체크
 * - 각 입력창 아래에 띄울 에러 메시지 제공
 * - allValid 로 가입 버튼 활성화/비활성화 제어
 *
 * confirm 필드가 필요 없으면(예: 직원 추가) confirm 관련 값은 무시하면 된다.
 */
export function useCredentialValidation(): CredentialState {
  ...
  return {
    ...
    passwordValid,
    confirmValid,
    allValid: usernameValid && passwordValid && confirmValid,
  }
}
